...
#define r3_node_connect(n, pat, child) r3_node_connectl(n, pat, (int)strlen(pat), 0, child)
...
node * r3_tree_matchl(const node * n, const char * path, int path_len, match_entry * entry, NSMutableDictionary *matchedParams);

#define r3_tree_match(n,p,e)  r3_tree_matchl(n,p, strlen(p), e)

// node * r3_tree_match_entry(node * n, match_entry * entry);
#define r3_tree_match_entry(n, entry) r3_tree_matchl(n, entry->path, entry->path_len, entry, NULL)
...
#if !USE_PCRE
NSString *r3_edge_slug_string(edge *e);
#endif
